...
function getBaseUrl(req) {
  // Prefer explicit BASE_URL from env (prevents host header injection).
  const fromEnv = (process.env.BASE_URL || '').trim();
  if (fromEnv) return stripTrailingSlash(fromEnv);

  // Fallback: derive from request (only for dev / non-production).
  if (req && req.headers) {
    const protoHeader = String(req.headers['x-forwarded-proto'] || req.protocol || 'http');
    const proto = protoHeader.split(',')[0].trim();
    const host = String(req.headers['x-forwarded-host'] || req.headers.host || '').split(',')[0].trim();
    // Sanity check: only allow safe host header values
