...
function requireAuthLoose(req, res, next) {
  if (!req.session || !req.session.userId) {
    // Gitmek istediği sayfayı kaydet (login sonrası geri dönebilsin)
    if (req.method === 'GET' && req.originalUrl && req.originalUrl.startsWith('/app')) {
      // Yeni bir session oluşturmayı dene (cookie-session otomatik yapar)
