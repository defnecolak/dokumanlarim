...
function hashBackupCode(codeOrObj, userId, pepper) {
  // Support both positional args: hashBackupCode(code, uId) and object form
  let c, u, p;
  if (codeOrObj && typeof codeOrObj === 'object' && !Array.isArray(codeOrObj)) {
    c = codeOrObj.code; u = codeOrObj.userId; p = codeOrObj.pepper;
  } else {
    c = codeOrObj; u = userId; p = pepper;
  }
  const norm = normalizeCode(c).toUpperCase();
...
module.exports = {
  ...
  generateSecretBase32: generateBase32Secret, // alias for server.js
  verifyTotp,
  totpVerify: function totpVerify(secret, code, opts) {
    // server.js calls totpVerify(secret, code, { window: 1 })
    // verifyTotp expects (code, secret, window)
    const w = (opts && typeof opts === 'object') ? (opts.window || 1) : (typeof opts === 'number' ? opts : 1);
    return verifyTotp(code, secret, w);
  },
  makeOtpAuthUrl,
  makeOtpauthURL: function makeOtpauthURL(opts) {
    // server.js passes { secretBase32, label, issuer }
    // makeOtpAuthUrl expects { issuer, accountName, secretBase32 }
    return makeOtpAuthUrl({
      issuer: opts.issuer || '',
      accountName: opts.label || opts.accountName || '',
      secretBase32: opts.secretBase32 || '',
    });
  },
  generateBackupCodes,
  hashBackupCode,
  timingSafeEqualHex,
  timingSafeEqualStr: timingSafeEqualHex,     // alias for server.js
  normalizeCode,
};
