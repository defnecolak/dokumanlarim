...
/**
 * SSRF protection: validate that webhook URLs are safe external URLs.
 * Block internal/private IPs, local hostnames, and non-HTTPS URLs.
 */
function isUrlSafe(urlStr) {
  try {
    const u = new URL(urlStr);
    // Must be HTTPS (or HTTP only for known webhook services)
...
async function postJson(url, payload, headers = {}, timeoutMs = 6000) {
  if (!isUrlSafe(url)) {
    return { ok: false, status: 0, text: 'url_blocked_ssrf_protection' };
  }
  ...
  try {
    const res = await fetch(url, {
      ...
      redirect: 'error', // Don't follow redirects (SSRF via redirect)
    });
